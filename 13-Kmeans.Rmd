...
1. Define 8 centers randomly, i.e pick 10 genes at random.
...
```{r}
centroids <- ycc[sample(1:256,8,replace=F),] # make a vector of random clusters
...
for(rounds in 1:100){ #do 100 iterations

  for(i in 1:256){
  
    for(j in 1:8){
      dists[i,j] <- e.dist(ycc[i,],centroids[j,]) # for each gene i calculate the distance to centroid J.
    }
  }
...
2. Write a function from this code that will take any `data`, any number of clusters `K`, and `iterations` as an argument and perform Kmeans clustering. Return a list that contains the cluster assignments and data.
